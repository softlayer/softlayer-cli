	...
	Describe("user device-access", func() {
		...
		Context("Return no error", func() {
			It("User set does not have devices and permissions", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(fakeUI.Outputs()).To(ContainSubstring("123456"))
				Expect(fakeUI.Outputs()).To(ContainSubstring("User does not have permissions about devices"))
				Expect(fakeUI.Outputs()).To(ContainSubstring("User does not have devices"))
			})
		})
	})
})
