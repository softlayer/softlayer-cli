...
var _ = Describe("Load balancer create", func() {
	...
	BeforeEach(func() {
		...
		cliCommand = cli.Command{
			Name:        loadbal.LoadbalOrderMetadata().Name,
			Description: loadbal.LoadbalOrderMetadata().Description,
			Usage:       loadbal.LoadbalOrderMetadata().Usage,
			Flags:       loadbal.LoadbalOrderMetadata().Flags,
			Action:      cmd.Run,
		}
	...
	Context("create without type", func() {
		It("return error", func() {
			err := testhelpers.RunCommand(cliCommand, "-n", "name", "-d", "dal09")
			Expect(err).To(HaveOccurred())
	...
	Context("create with incorrect type", func() {
		It("return error", func() {
			err := testhelpers.RunCommand(cliCommand, "-n", "name", "-d", "dal09", "-t", "abcd")
			Expect(err).To(HaveOccurred())
	...
	Context("create without subnet", func() {
		It("return no error", func() {
			err := testhelpers.RunCommand(cliCommand, "-n", "1234", "-t", "publictoprivate", "-d", "dal09", "-f")
			Expect(err).To(HaveOccurred())
	...
	Context("create load balancer with incorrect sticky", func() {
		It("return error", func() {
			err := testhelpers.RunCommand(cliCommand, "-n", "1234", "-t", "publictoprivate", "-d", "dal09", "-s", "123", "-c", "3", "--sticky", "abcd", "-f")
			Expect(err).To(HaveOccurred())
	...
	Context("create with correct load balancer ID", func() {
		It("return no error", func() {
			err := testhelpers.RunCommand(cliCommand, "-n", "1234", "-t", "publictoprivate", "-d", "dal09", "-s", "123", "-c", "3", "--sticky", "cookie", "-f")
			Expect(err).NotTo(HaveOccurred())
	...
	Context("verify create load balancer", func() {
		It("return no error", func() {
			err := testhelpers.RunCommand(cliCommand, "-n", "1234", "-t", "publictoprivate", "-d", "dal09", "-s", "123", "-c", "3", "--sticky", "source-ip", "--verify")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstring("OK"))
			Expect(fakeUI.Outputs()).To(ContainSubstring("Item                      Cost"))
	...
	Context("create load balancer with type publictopublic and subnet different to 0", func() {
		It("return error", func() {
			err := testhelpers.RunCommand(cliCommand, "-n", "1234", "-t", "publictopublic", "-d", "dal09", "-s", "123", "-f")
			Expect(err).To(HaveOccurred())
	...
	Context("create load balancer with type privatetoprivate and public subnet", func() {
		It("return error", func() {
			err := testhelpers.RunCommand(cliCommand, "-n", "1234", "-t", "privatetoprivate", "-d", "dal09", "-s", "123", "--use-public-subnet", "-f")
			Expect(err).To(HaveOccurred())
