...
var _ = Describe("VS resume", func() {
	var (
		...
		cliCommand    *virtual.ResumeCommand
	...
	BeforeEach(func() {
		...
		cliCommand = virtual.NewResumeCommand(slCommand)
	...
	Describe("VS resume", func() {
		...
		Context("VS resume with correct vs ID but not continue", func() {
			It("return no error", func() {
				...
				Expect(fakeUI.Outputs()).To(ContainSubstring("This will resume virtual server instance: 1234. Continue?"))
		...
		Context("VS resume with correct vs ID but server fails", func() {
			...
			It("return error", func() {
				...
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("Failed to resume virtual server instance: 1234."))
		...
		Context("VS resume with correct vs ID ", func() {
			...
			It("return no error", func() {
				...
				Expect(fakeUI.Outputs()).To(ContainSubstring("Virtual server instance: 1234 was resumed."))
			})
		})
	})
})
