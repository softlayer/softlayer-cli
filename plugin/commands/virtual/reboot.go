...
func NewRebootCommand(sl *metadata.SoftlayerCommand) (cmd *RebootCommand) {
	...
	cobraCmd := &cobra.Command{
		...
		Short: T("Reboot an active virtual server instance"),
...
func (cmd *RebootCommand) Run(args []string) error {
	...
	err = cmd.VirtualServerManager.RebootInstance(vsID, cmd.Soft, cmd.Hard)
	if err != nil {
		return slErrors.NewAPIError(T("Failed to reboot virtual server instance: {{.VsID}}.\n", subs), err.Error(), 2)
	}
	cmd.UI.Ok()
