...
var _ = Describe("VS pause", func() {
	var (
		...
		cliCommand    *virtual.PauseCommand
	...
	BeforeEach(func() {
		...
		cliCommand = virtual.NewPauseCommand(slCommand)
	...
	Describe("VS pause", func() {
		...
		Context("VS pause with correct vs ID but not continue", func() {
			It("return no error", func() {
				...
				Expect(fakeUI.Outputs()).To(ContainSubstring("This will pause virtual server instance: 1234. Continue?"))
		...
		Context("VS pause with correct vs ID but server fails", func() {
			...
			It("return error", func() {
				...
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("Failed to pause virtual server instance: 1234."))
		...
		Context("VS pause with correct vs ID ", func() {
			...
			It("return no error", func() {
				...
				Expect(fakeUI.Outputs()).To(ContainSubstring("Virtual server instance: 1234 was paused."))
			})
		})
	})
})
