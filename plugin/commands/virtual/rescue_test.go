...
var _ = Describe("VS rescue", func() {
	var (
		...
		cliCommand    *virtual.RescueCommand
	...
	BeforeEach(func() {
		...
		cliCommand = virtual.NewRescueCommand(slCommand)
	...
	Describe("VS rescue", func() {
		...
		Context("VS rescue with correct vs ID but server fails", func() {
			...
			It("return error", func() {
				...
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("Failed to rescue virtual server instance: 1234."))
