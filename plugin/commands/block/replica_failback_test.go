...
var _ = Describe("Replica failback", func() {
	var (
		...
		cliCommand         *block.ReplicaFailbackCommand
	...
	BeforeEach(func() {
		...
		cliCommand = block.NewReplicaFailbackCommand(slCommand)
	...
	Describe("Replicant failback", func() {
		...
		Context("Replicant fail back with correct volume id", func() {
			It("return no error", func() {
				...
				Expect(fakeUI.Outputs()).To(ContainSubstring("Failback of volume 1234 is now in progress."))
			})
		})
		...
		Context("Replicant fail back with correct volume id but server API call fails", func() {
			...
			It("return error", func() {
				...
				Expect(err.Error()).To(ContainSubstring("Failback operation could not be initiated for volume 1234."))
			})
		})
	})
})
