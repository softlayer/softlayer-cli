...
var _ = Describe("Order preset-list", func() {
	var (
		...
		cmd              *order.PresetListCommand
		cliCommand       cli.Command
	)
	BeforeEach(func() {
		fakeOrderManager = new(testhelpers.FakeOrderManager)
		fakeUI = terminal.NewFakeUI()
		cmd = order.NewPresetListCommand(fakeUI, fakeOrderManager)
		cliCommand = cli.Command{
			Name:        order.OrderPresetListMetaData().Name,
			Description: order.OrderPresetListMetaData().Description,
			Usage:       order.OrderPresetListMetaData().Usage,
			Flags:       order.OrderPresetListMetaData().Flags,
			Action:      cmd.Run,
		}
	})
