	...
	Describe("Account events", func() {
		...
		Context("Account events, correct use", func() {
			...
			It("return account events in format json", func() {
				...
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"Id": "112238162",`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"Hostname": "resource.softlayer.test",`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"Label": "Capacity - Windows"`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`======= Update #1 on 2022-03-23T00:50:57Z =======`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`Updated message`))
