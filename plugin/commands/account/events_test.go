	...
	Describe("Account events", func() {
		...
		Context("Account events, correct use", func() {
			...
			It("return account events in format json", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"acknowledgedFlag": false,`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"endDate": "2022-04-08T00:00:00-06:00",`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"id": 341058,`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"impactedResourceCount": 2,`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"modifyDate": "2022-03-24T11:34:32-06:00",`))
				Expect(fakeUI.Outputs()).To(ContainSubstring(`"notificationOccurrenceEventType": {`))
