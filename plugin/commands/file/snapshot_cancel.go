...
func (cmd *SnapshotCancelCommand) Run(args []string) error {
	...
	if !cmd.Force {
		confirm, err := cmd.UI.Confirm(T("This will cancel the file volume snapshot space: {{.ID}} and cannot be undone. Continue?", subs))
		if err != nil {
			return err
		}
		if !confirm {
	...
	err = cmd.StorageManager.CancelSnapshotSpace("file", volumeID, cmd.Reason, cmd.Immediate)
	if err != nil {
	...
	cmd.UI.Ok()
	if cmd.Immediate {
		cmd.UI.Print(T("File volume {{.ID}} has been marked for immediate snapshot cancellation.", subs))
	} else {
		cmd.UI.Print(T("File volume {{.ID}} has been marked for snapshot cancellation.", subs))
	}
	return nil
}
