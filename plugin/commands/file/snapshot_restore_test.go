...
var _ = Describe("Snapshot restore", func() {
	...
	BeforeEach(func() {
		...
		cliCommand = cli.Command{
			Name:        file.FileSnapshotRestoreMetaData().Name,
			Description: file.FileSnapshotRestoreMetaData().Description,
			Usage:       file.FileSnapshotRestoreMetaData().Usage,
			Flags:       file.FileSnapshotRestoreMetaData().Flags,
			Action:      cmd.Run,
		}
	})
