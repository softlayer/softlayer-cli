...
var _ = Describe("Volume options", func() {
	var (
		...
		cmd                *file.VolumeOptionsCommand
		cliCommand         cli.Command
	)
	BeforeEach(func() {
		fakeUI = terminal.NewFakeUI()
		FakeStorageManager = new(testhelpers.FakeStorageManager)
		cmd = file.NewVolumeOptionsCommand(fakeUI, FakeStorageManager)
		cliCommand = cli.Command{
			Name:        metadata.FileVolumeOptionsMetaData().Name,
			Description: metadata.FileVolumeOptionsMetaData().Description,
			Usage:       metadata.FileVolumeOptionsMetaData().Usage,
			Flags:       metadata.FileVolumeOptionsMetaData().Flags,
			Action:      cmd.Run,
		}
	})
	...
	Describe("Volume options", func() {
		...
		Context("Volume options", func() {
			...
			It("return no error", func() {
				...
				Expect(fakeUI.Outputs()).NotTo(ContainSubstrings([]string{"OS Type"}))
				...
				Expect(fakeUI.Outputs()).NotTo(ContainSubstrings([]string{"HYPER_V,LINUX,VMWARE,WINDOWS_2008,WINDOWS_GPT,WINDOWS,XEN"}))
