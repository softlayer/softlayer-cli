...
EXAMPLE:
   ...
   This command authorizes virtual server with ID 87654321 to access volume with ID 12345678.`, sl.StorageI18n),
	...
	cobraCmd.Flags().IntSliceVarP(&thisCmd.Subnet_id, "subnet-id", "s", []int{}, T("An ID of one subnet to authorize"))
...
func (cmd *AccessAuthorizeCommand) Run(args []string) error {
	...
	resp, err := cmd.StorageManager.AuthorizeHostToVolume(volumeID, cmd.Hardware_id, cmd.Virtual_id, IPIds, cmd.Subnet_id)
	if err != nil {
