...
var _ = Describe("File Volume Set Note", func() {
	var (
	...
	BeforeEach(func() {
		...
		cliCommand = cli.Command{
			...
			Usage:       file.FileVolumeSetNoteMetaData().Usage,
			Flags:       file.FileVolumeSetNoteMetaData().Flags,
			Action:      cmd.Run,
		}
	})

	Describe("File Volume Set Note", func() {
		...
		Context("Proper Usage", func() {
			BeforeEach(func() {
				FakeStorageManager.VolumeSetNoteReturns(false, errors.New("Error occurred while note was adding in file volume"))
			})
			It("return error", func() {
				err := testhelpers.RunCommand(cliCommand, "1234", "--note=thisismynote")
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("Error occurred while note was adding in file volume"))
			})
		})
