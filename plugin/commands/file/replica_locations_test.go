...
var _ = Describe("Replica locations", func() {
	var (
		...
		cmd                *file.ReplicaLocationsCommand
		cliCommand         cli.Command
	)
	BeforeEach(func() {
		fakeUI = terminal.NewFakeUI()
		FakeStorageManager = new(testhelpers.FakeStorageManager)
		cmd = file.NewReplicaLocationsCommand(fakeUI, FakeStorageManager)
		cliCommand = cli.Command{
			Name:        metadata.FileReplicaLocationsMetaData().Name,
			Description: metadata.FileReplicaLocationsMetaData().Description,
			Usage:       metadata.FileReplicaLocationsMetaData().Usage,
			Flags:       metadata.FileReplicaLocationsMetaData().Flags,
			Action:      cmd.Run,
		}
	})
	...
	Describe("Replicant locations", func() {
		...
		Context("Replicant locations", func() {
			...
			It("succeed", func() {
				err := testhelpers.RunCommand(cliCommand, "1234")
				Expect(err).NotTo(HaveOccurred())
				Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"No data centers compatible for replication."}))
			})
		})
	})
})
