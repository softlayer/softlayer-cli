...
var _ = Describe("IPSec update", func() {
	var (
		...
		cliCommand       *ipsec.UpdateCommand
	...
	Context("update with wrong phase1-auth", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-a", "abc")
			Expect(err).To(HaveOccurred())
	...
	Context("update with wrong phase1-crypto", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-c", "abc")
			Expect(err).To(HaveOccurred())
	...
	Context("update with wrong phase1-dh", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-d", "10")
			Expect(err).To(HaveOccurred())
	...
	Context("update with wrong phase1-key-ttl", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-t", "100")
			Expect(err).To(HaveOccurred())
	...
	Context("update with wrong phase2-auth", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-u", "abc")
			Expect(err).To(HaveOccurred())
	...
	Context("update with wrong phase2-crypto", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-y", "abc")
			Expect(err).To(HaveOccurred())
	...
	Context("update with wrong phase2-dh", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-e", "3")
			Expect(err).To(HaveOccurred())
	...
	Context("update with wrong phase2-forward-secrecy", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-f", "2")
			Expect(err).To(HaveOccurred())
	...
	Context("update with wrong phase2-key-ttl", func() {
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-l", "100")
			Expect(err).To(HaveOccurred())
	...
	Context("update with server fail", func() {
		...
		It("return error", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-l", "150")
			Expect(err).To(HaveOccurred())
	...
	Context("update", func() {
		...
		It("succeed", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-a", "SHA256")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"Updated IPSec 1234."}))
		})
		It("succeed", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-c", "AES256")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"Updated IPSec 1234."}))
		})
		It("succeed", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-d", "1")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"Updated IPSec 1234."}))
		})
		It("succeed", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-t", "1000")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"Updated IPSec 1234."}))
		})
		It("succeed", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-u", "SHA256")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"Updated IPSec 1234."}))
		})
		It("succeed", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-y", "AES256")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"Updated IPSec 1234."}))
		})
		It("succeed", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-e", "2")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"Updated IPSec 1234."}))
		})
		It("succeed", func() {
			err := testhelpers.RunCobraCommand(cliCommand.Command, "1234", "-f", "1")
			Expect(err).NotTo(HaveOccurred())
			Expect(fakeUI.Outputs()).To(ContainSubstrings([]string{"Updated IPSec 1234."}))
		})
		It("succeed", func() {
