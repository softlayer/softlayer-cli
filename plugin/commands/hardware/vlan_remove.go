...
func NewVlanRemoveCommand(sl *metadata.SoftlayerCommand) (cmd *VlanRemoveCommand) {
	...
	cobraCmd := &cobra.Command{
		Use:   "vlan-remove " + T("IDENTIFIER") + " " + T("VLAN") + "...",
...
func (cmd *VlanRemoveCommand) Run(args []string) error {
	...
	if len(pri_vlans) > 0 {
		for _, component := range hardware.BackendNetworkComponents {
			if component.PrimaryIpAddress != nil {
				added_vlans, err := cmd.HardwareManager.UnTrunkVlans(*component.Id, pri_vlans)
				if err != nil {
					return err
				}
				for _, v := range added_vlans {
